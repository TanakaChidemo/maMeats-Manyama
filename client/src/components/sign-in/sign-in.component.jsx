...
const SignIn = () => {
    ...
    const sendEmailLoginData = async (emailID, password) => {
        try {
            const response = await fetch('http://localhost:8000/test/users/loginWithEmail', {
                method: 'POST',
                headers: {
    ...
    const sendPhoneLoginData = async (phoneNumber, password) => {
        try {
            const response = await fetch('http://localhost:3000/test/users/loginWithPhone', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let result;
            if (signInMethod === 'email') {
                result = await sendEmailLoginData(emailID, password);
            } else {
                        ...
                        <div className="px-4 py-6 sm:px-8">
                            <div className="flex items-center justify-between">
                                ...
                                <p className="text-base font-normal text-gray-900 font-pj">
                                    ...
                                    <Link to="/sign-up" title="" className="font-bold rounded hover:underline focus:outline-none focus:ring-1 focus:ring-gray-900 focus:ring-offset-2">
                                        Join now
                                    </Link>
                                </p>
                            </div>
